...
class SessionExamEncoding(models.TransientModel):
    ...
    def _get_default_session_number(self):
        # self.academic_year.academic_year_id.start_date
        # today = datetime.now()
    ...
    session_month_selection = fields.Selection([(1,'Session 1'),(2,'Session 2'),(3,'Session 3')], default = _get_default_session_number)
    ...
    def _set_session_list(self):
        if self.tutor and self.academic_year and self.session_month_selection:
            print '_set_session_list if' + str(self.tutor.id) + ", " + str(self.academic_year.id) + ", " + str(self.session_month_selection)
