...
def learning_units_search(request):
    ...
    learning_units = LearningUnitYear.objects.filter(academic_year=int(academic_year))

    if not(code is None) and len(code) > 0  :
        learning_units = learning_units.filter(acronym__istartswith=code)
...
def learning_unit_read(request,learning_unit_id):
    learning_unit_year = LearningUnitYear.objects.get(pk=learning_unit_id)
