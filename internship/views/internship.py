...
def work_dist(student, organizations):
    ...
    student_informations = mdl_internship_student_information.search(person__last_name=student.person.last_name, person__first_name=student.person.first_name)

    distance_student_organization = {}
    # For each organization in the list find the informations
    for organization in organizations :
        organization_informations = mdl_organization_address.search(organization = organization)
...
def get_number_choices(internships):
    ...
    for internship in internships:
        number_first_choice = len(mdl_internship_choice.search(organization = internship.organization,
                                                            speciality__acronym = internship.speciality.acronym,
                                                           choice=1))
        number_other_choice = len(mdl_internship_choice.search_other_choices(organization = internship.organization,
                                                            speciality__acronym = internship.speciality.acronym))
...
def set_tabs_name(specialities, student=None):
    ...
    for speciality in specialities:
        if student :
            size = len(mdl_internship_choice.search(speciality=speciality, student=student))
...
def sort_internships(sort_internships):
    ...
    for i in number_ref:
        internships = mdl_internship_offer.search(organization__reference=i)
        for internship in internships:
...
def internships(request):
    ...
    if organization_sort_value and organization_sort_value != "0":
        query = mdl_internship_offer.search(organization__name = organization_sort_value)
    else:
        query = mdl_internship_offer.find_internships()
    ...
    all_internships = mdl_internship_offer.find_internships()
    ...
    all_non_mandatory_speciality = mdl_internship_speciality.find_non_mandatory()
    if speciality_sort_value:
        all_non_mandatory_internships = mdl_internship_offer.find_non_mandatory_internships(speciality__name=speciality_sort_value)
    else:
        all_non_mandatory_internships = mdl_internship_offer.find_non_mandatory_internships(speciality__mandatory=0)
...
def internships_stud(request):
    ...
    student_choice = mdl_internship_choice.find_by_student_desc(student)
    ...
    for x in range(0,size_non_mandatory):
        if speciality_sort_value[x]:
            all_non_mandatory_internships[x] = mdl_internship_offer.find_non_mandatory_internships(speciality__name=speciality_sort_value[x])
        ...
        all_non_mandatory_selected_internships[x]=mdl_internship_choice.search(internship_choice=x+1)
...
def internships_save(request):
    ...
    all_internships = mdl_internship_offer.search()
    selectable = get_selectable(all_internships)
    ...
    if selectable :
        ...
        mdl_internship_choice.InternshipChoice.objects.filter(student=student, internship_choice=0).delete()
        ...
        if request.POST.getlist('is_choice'):
            ...
            for choice_tab in internship_choice_tab_del:
                mdl_internship_choice.InternshipChoice.objects.filter(student=student, internship_choice=choice_tab).delete()
        ...
        for x in range(0, index):
            new_choice = mdl_internship_choice.InternshipChoice()
            new_choice.student = student[0]
            organization = mdl_organization.search(reference=organization_list[x])
            new_choice.organization = organization[0]
            speciality = mdl_internship_speciality.search(name=speciality_list[x])
...
def student_choice(request, id):
    ...
    internship = mdl_internship_offer.find_intership_by_id(id)
    # Get the students who have choosen this internship
    students = mdl_internship_choice.search(organization=internship.organization,
                                       speciality=internship.speciality)
    number_choices = [None]*5

    # Get the choices' number for this internship
    for index in range(1, 5):
        number_choices[index] = len(mdl_internship_choice.search(organization=internship.organization,
                                                            speciality=internship.speciality,
                                                            choice=index))
...
def internships_block(request):
    internships = mdl_internship_offer.search()
    # For each internship in the DB invert the selectable flag
    for internship in internships:
        edit_internship = mdl_internship_offer.find_intership_by_id(internship.id)
...
def internships_modification_student(request, registration_id):
    ...
    # Get in descending order the student's choices in first lines
    student_choice = mdl_internship_choice.find_by_student_desc(student)
    student_enrollment = mdl_internship_enrollment.search(student = student)
...
def internship_save_modification_student(request) :
    ...
    mdl_internship_choice.InternshipChoice.objects.filter(student=student).delete()
    mdl_internship_enrollment.InternshipEnrollment.objects.filter(student=student).delete()
    ...
    for x in range(0, index):
        if periods_list[x] != '0':
            new_enrollment = mdl_internship_enrollment.InternshipEnrollment()
            tab_period = periods_list[x].split('\\n')
            period = mdl_period.search(name=tab_period[0])
            organization = mdl_organization.search(reference=tab_period[1])
            speciality = mdl_internship_speciality.search(name=tab_period[2])
            internship = mdl_internship_offer.search(speciality__name = speciality[0], organization__reference = organization[0].reference)
