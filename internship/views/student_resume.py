...
def set_number_choices(student_informations):
    ...
    for si in student_informations:
        ...
        choices = mdl_internship_choice.find_by_student(student)
...
def internships_student_resume(request):
    ...
    students_list = mdl_internship_choice.find_by_all_student()
    specialities = mdl_internship_speciality.search(mandatory=True)
    student_informations = mdl_internship_student_information.find_all()
    ...
    students_can_have_internships = len(mdl_internship_student_information.find_all())
...
def internships_student_read(request, registration_id):
    ...
    information = mdl_internship_student_information.search(person = student_to_read.person)
    internship_choice = mdl_internship_choice.find_by_student(student_to_read)
    all_speciality = mdl_internship_speciality.search(mandatory=True)

    affectations = mdl_internship_student_affectation_stat.search(student=student_to_read).order_by("period__date_start")
    ...
    internships = mdl_internship_offer.find_internships()
...
def internship_student_information_modification(request, registration_id):
    ...
    information = mdl_internship_student_information.search(person = student[0].person)
    student = student[0]
...
def student_save_information_modification(request, registration_id):
    ...
    information = mdl_internship_student_information.search(person = student[0].person)
    if not information:
        information = mdl_internship_student_information.InternshipStudentInformation()
...
def internship_student_affectation_modification(request, student_id):
    informations = mdl_internship_student_affectation_stat.search(student__pk = student_id)
    internship_choice = mdl_internship_choice.search(student__pk = student_id)
    if not internship_choice:
        student = mdl.student.find_by_id(student_id)
        information = mdl_internship_choice.InternshipChoice()
    ...
    specialities = mdl_internship_speciality.find_all()
...
def student_save_affectation_modification(request, registration_id):
    ...
    for x in range(0, index):
        if organization_list[x] != "0":
            organization = mdl_organization.search(reference=organization_list[x])[0]
            speciality = mdl_internship_speciality.search(name=speciality_list[x])[0]
            period = Period.search(name=period_list[x])[0]
            check_internship_present = mdl_internship_offer.search(organization=organization, speciality=speciality)
    ...
    if not check_error_present:
        mdl_internship_student_affectation_stat.search(student=student).delete()
        index = len(period_list)
        for x in range(0, index):
            if organization_list[x] != "0":
                organization = mdl_organization.search(reference=organization_list[x])[0]
                speciality = mdl_internship_speciality.search(name=speciality_list[x])[0]
                period = Period.search(name=period_list[x])[0]
                student_choices = mdl_internship_choice.search(student=student, speciality=speciality)
                affectation_modif = mdl_internship_student_affectation_stat.InternshipStudentAffectationStat()
